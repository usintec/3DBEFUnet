...
# Optional: try importing elasticdeform, fallback if unavailable
try:
    import elasticdeform
    ELASTIC_AVAILABLE = True
except ImportError:
    ELASTIC_AVAILABLE = False
    print("⚠️ elasticdeform not available, skipping elastic augmentation.")
...
def crop_foreground(modalities, seg, crop_size=(96,96,96)):
    """Tumor-aware cropping around non-zero voxels."""
    non_zero = np.argwhere(seg > 0)
    if len(non_zero) > 0:
        center = non_zero[np.random.choice(len(non_zero))]
    else:
        center = [random.randint(crop_size[i]//2, seg.shape[i]-crop_size[i]//2) for i in range(3)]
...
def augment(modalities, seg, target_shape=(96,96,96)):
    ...
    # Random gamma correction (clip before power)
    if random.random() > 0.5:
        gamma = random.uniform(0.8,1.2)
        modalities = np.clip(modalities, 1e-6, None)
    ...
    if ELASTIC_AVAILABLE and random.random() > 0.5:
...
class BraTSDataset(Dataset):
    def __init__(self, case_dirs, transform=True, target_shape=(128,128,128), crop_size=(96,96,96)):
        ...
        print(f"Loaded {len(case_dirs)} cases | Transform={transform}")
    ...
    def __getitem__(self, idx):
        ...
        # Load modalities
        modality_files = {
            m: glob.glob(os.path.join(case_dir, f"*{m}.nii*"))[0]
...
def make_weighted_sampler(dataset):
    """Weighted sampler based on tumor fraction for class imbalance."""
    weights = []
