...
def trainer_3d_backup_complete_save_resumption(args, model, snapshot_path):
    import datetime
    date_and_time = datetime.datetime.now()
    os.makedirs(os.path.join(snapshot_path, 'test'), exist_ok=True)
    test_save_path = os.path.join(snapshot_path, 'test')
    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            try:
                ...
                if iter_num % 200 == 0:  # 🔑 save checkpoint frequently
                    save_checkpoint(
                        {
                            "model_state": model.state_dict(),
                            "optimizer_state": optimizer.state_dict(),
        ...
        save_checkpoint(
            ...
            snapshot_path,
            f"{args.model_name}_epoch{epoch_num}.pth",
        )
...
def trainer_3d_no_complete_save_resumption(args, model, snapshot_path):
    """
    3D trainer with CE + Dice + Class-wise Discriminative Loss (DLF).
